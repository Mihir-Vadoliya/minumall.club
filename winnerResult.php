...
$manualResultNumberRow = mysqli_num_rows($manualresultQuery);

if($manualResultNumberRow > 0){
...
if ($chkResultRow == 0) {
    ...
    if ($selectdataRow != '' &&  $manualResultNumberRow > 0) {
        if ($type1 == 'parity') {
            ...
            if ($chkparityRow > 0) {
                ...
                if ($switchstatus == 'yes' && $switchtab == 'parity') {
                    ...
                    $tempwinQuery = mysqli_query($con, "SELECT `number`,`color`,`total` from `tbl_tempwinner` where `total` = (select min(`total`)from `tbl_tempwinner` where `total`>=0)and `periodid`='" . $periodid . "' and `type`='parity' limit 1");
                }
        ...
        if ($type2 == 'sapre') {
            ...
            if ($chkparityRow > 0) {
                ...
                if ($switchstatus == 'yes' && $switchtab == 'sapre') {
                    ...
                    $tempwinQuery = mysqli_query($con, "SELECT `number`,`color`,`total` from `tbl_tempwinner` where `total` = (select min(`total`)from `tbl_tempwinner` where `total`>=0)and `periodid`='" . $periodid . "' and `type`='sapre' limit 1");
                }
                $tempwinResult = mysqli_fetch_array($tempwinQuery);
                $tempNumber = $tempwinResult['number'];
                $tempColor = $tempwinResult['color'];
        ...
        if ($type3 == 'bcone') {
            ...
            if ($chkparityRow  > 0 ) {
                ...
                if ($switchstatus == 'yes' && $switchtab == 'bcone') {
                    ...
                    $tempwinQuery = mysqli_query($con, "SELECT `number`,`color`,`total` from `tbl_tempwinner` where `total` = (select min(`total`)from `tbl_tempwinner` where `total`>=0)and `periodid`='" . $periodid . "' and `type`='bcone' limit 1");
                }
                $tempwinResult = mysqli_fetch_array($tempwinQuery);
                $tempNumber = $tempwinResult['number'];
                $tempColor = $tempwinResult['color'];
        ...
        if ($type4 == 'emerd') {
            ...
            if ($chkparityRow  > 0 ) {
                ...
                if ($switchstatus == 'yes' && $switchtab == 'emerd') {
                    ...
                    $tempwinQuery = mysqli_query($con, "SELECT `number`,`color`,`total` from `tbl_tempwinner` where `total` = (select min(`total`)from `tbl_tempwinner` where `total`>=0)and `periodid`='" . $periodid . "' and `type`='emerd' limit 1");
                }
                $tempwinResult = mysqli_fetch_array($tempwinQuery);
                $tempNumber = $tempwinResult['number'];
                $tempColor = $tempwinResult['color'];
        ...
        //=================random data=======================================
        $tabtypearray = array('parity', 'sapre', 'bcone', 'emerd');
        for ($i = 0;$i <= 3;$i++) {
