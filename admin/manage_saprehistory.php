...
<head>
  ...
  <title>Adminsuit | History For Sapre</title>
 ...
 <?php include ("include/navigation.inc.php");?> 
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>Today's History For Sapre</h1>
      <ol class="breadcrumb">
        <li><a href="desktop.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Today's History For Sapre</li>
      </ol>
    </section>
...
$todayshow=date('d/m/Y');
  $Query=mysqli_query($con,"select * from `tbl_result` where  `tabtype`='sapre' order by id desc");
  while($row=mysqli_fetch_array($Query)){
$Query2=mysqli_query($con,"select `userid`,sum(`amount`)as tradeamount from `tbl_userresult` where `periodid`='".$row['periodid']."' and `tab`='sapre' group by userid");
