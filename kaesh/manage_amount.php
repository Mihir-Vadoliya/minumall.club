...
if(isset($_POST['submit'])=='Submit'){
	...
	$level1=($_POST['level1']);
	$level2=($_POST['level2']);
    $with_charge=$_POST['with_charge'];
    $first_bonus=$_POST['first_bonus'];
	
$sql= "UPDATE `tbl_paymentsetting` SET `rechargeamount` = '$mra',`withdrawalamount` = '$mwa',`level1` = '$level1',`level2` = '$level2',`with_charge`='$with_charge',`first_bonus`='$first_bonus' WHERE `id`= '1'";
...
<!--              </div>-->
 <div class="col-sm-6">
              <div class="form-group">
              <label>Level1 Commission <i class="red_txt">[in %]</i></label>
              <input type="text" class="form-control" name="level1" id="level1" required value="<?php echo $role['level1'];?>">
              </div>
              </div>
<div class="col-sm-6">
              <div class="form-group">
              <label>Level2 Commission <i class="red_txt">[in %]</i> </label>
              <input type="text" class="form-control"  name="level2" id="level2" required value="<?php echo $role['level2'];?>">
              </div>
              </div>
               <div class="col-sm-6">
              <div class="form-group">
              <label>Withdrawal Charges<i class="red_txt">[in %]</i></label>
              <input type="text" class="form-control" name="with_charge" id="with_charge" required value="<?php echo $role['with_charge'];?>">
              </div>
              </div>
              <div class="col-sm-6">
              <div class="form-group">
              <label>First Recharge Bonus </label>
              <input type="text" class="form-control"  name="first_bonus" id="first_bonus" required value="<?php echo $role['first_bonus'];?>">
              </div>
              </div>
             <div class="clearfix"></div>   
              <div class="form-group">
